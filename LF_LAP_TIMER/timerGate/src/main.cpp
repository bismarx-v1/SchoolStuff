...
void loop() {
  ...
  switch(state) {
    case STATE_WAITING:
      STATE_ON_ENTER(
      state, statePrev, for(uint8_t idx = 0; idx < 7; idx++) { displayBuffer[idx] = "WAITING"[idx]; } displayBufferChanged = 1; keepFrame = 0;)

      if(armChange == 1 && armVal == 0) {  // Arm button.
        state = STATE_ARMED;
      }

      STATE_ON_EXIT(state, statePrev,  keepFrame = 0;)
      break;

    case STATE_ARMED:
      STATE_ON_ENTER(
      state, statePrev, for(uint8_t idx = 0; idx < 7; idx++) { displayBuffer[idx] = "ARMED  "[idx]; } displayBufferChanged = 1;  keepFrame = 0;)
    ...
    case STATE_RUNNING:
      STATE_ON_ENTER(
      state, statePrev, for(uint8_t idx = 0; idx < 9; idx++) { displayBuffer[idx + DISPLAY_TIMER_OFFSET] = "000:00:00"[idx];  keepFrame = 0;})
    ...
    case STATE_END:
      STATE_ON_ENTER(state, statePrev,  keepFrame = 1;)
